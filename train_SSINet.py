...
from model.SSINet import SSINet
...
def train(train_loader, model, optimizer, epoch):
    ...
    for i, pack in enumerate(train_loader, start=1):
        for rate in size_rates:
           ...
           s1, s1_sig, s2, s2_sig, s3, s3_sig, s4, s4_sig, s5, s5_sig, edge1 = model(images)
           # bce+iou+fmloss
    ...
    save_path = 'models/SSINet/'
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    if (epoch+1) >= 41:
        torch.save(model.state_dict(), save_path + 'SSINet_' + opt.trainset + '.pth' + '.%d' % epoch)
